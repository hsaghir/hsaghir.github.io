...
## RL from philosophical perspective
...
these three things cannot coexist. so we have to make tradeoffs.
1. Generalization
2. Off-Policy
3. Boostrapping
...
## RL from machine learning perspective

We can think of RL as a structured semi-supervised problem. whereas in supervised learning one has a label for each training example and in unsupervised learning one has no labels at all, in RL one has sparse and time-delayed labels – i.e. the rewards. 

However, in (semi)supervised learning, we usually have an iid assumption between samples of the dataset where in RL, the samples are trajectories where iid assumption doesn't hold. However, if we have a structured prediction supervised learning problem (e.g. image captioning), then we have dependencies across the sequence of predictions for each sample as well. In both cases however, we want to learn a mapping from inputs or states to outputs or actions:
    + In supervised learning, we can sample from model, calculate error and optimize parameters. What need to be concerned about is over-fitting and model being certain when it hasn't seen enough data points. 
...
We want to learn a mapping $$X \to a$$:
    ...
    + that maximizes a non-decomposable expected reward (objective) function $$r(a|X)$$
        ...
        - In RL, instead of target values, we have a bunch of raters that rate a sequence of actions (reward function) that may be sparse (not available for all inputs), and also may be delayed till the end. 
    ...
    + The policy or model that maximizes the expected reward or objective, $$E[r(a|X)]$$, is the optimal policy/model $$\pi^* (a|x)$$. If actions are discrete, the optimal policy is represented using a categorical variable. We can use the softmax trick to make it continuous with a temprature parameter ($$\tau$$), i.e. $$\pi^*(a|x) = \frac{1}{Z} \exp(\frac{r(a|x)}{\tau})$$.
        - If we use the soft optimal model, the conditional log-likelihood of the ML model is exactly the KL divergence between the optimal model and the model i.e. $$KL[\frac{\pi^* (a|x)}{\pi_\theta (a|x)}]$$ when the temprature parameter is zero.
        - If we use the soft optimal policy, the expected reward of RL is exactly the KL divergence in the other direction, i.e. between the policy function and the optimal policy i.e. $$KL[\frac{\pi_\theta (a|x)}{\pi^* (a|x)}]$$ again at temperature zero. 

In classical supervised learning, we minimize the conditional log-likelihood objective function, while in RL, we optimize the expected reward function. These two are actually pretty much very close concepts if we consider uncertainty. Using a distribution to represent the optimal model and optimal policy instead of a single point estimate, a divergence between the optimal and the current model/ploicy in different directions forms the objective functions in the two domains of RL and ML. The key question is what divergence functions can we use to better do this optimization in both domains. 
...
### [RL as Probabilistic Inference](https://arxiv.org/pdf/1805.00909.pdf)

- Conventionally, RL has been cast into a framework that aims to generalize probabilistic models by augmenting them with utilities or rewards, where the reward function is viewed as an extrinsic signal. In this view, determining a policy is a fundamentally distinct type of problem than probabilistic inference, although the underlying dynamical system might still be described by a probabilistic graphical model. The paper introduces an alternate view of RL, where the decision making problem is simply an inference problem in a particular type of graphical model. It argues max entropy RL is equivalent to exact probabilistic inference in the case of deterministic dynamics, and variational inference in the case of stochastic dynamics. In this view, the reward induces a distribution over random
variables, and the optimal policy aims to explicitly match a probability distribution defined by the reward and system dynamics, which may in future work suggest a way to systematize reward design.
...
Suppose you are an agent, situated in an environment. The environment is in a certain state that the agent can observe. The agent then performs an action which changes the state of the environment. The actions sometimes result in a reward signal to the agent that may be sparse or delayed. The goal of the agent is to maximize the average sum of its rewards over a set of episodes.

- some definitions:
    - State: data points $$X = {x_0, x_1, ... x_n}$$. We define it in a way that it contains sufficient information for predicting an action. 
    - The environment consists of a set of possible states $$S = {s_0, s_1, ... s_n}$$ and a transition function. 
    ...
    -  Model: Agent's model of the environment that is usually unknown (conditional probability of next state given past states)
    ...
    -  Value Function assesses how good each state/action is. It is the averaged future reward accumulated from each particular (state,action) or Expected reward. If the problem is deterministic expected reward is just the same as return. 
    -  credit assignment problem: which of the preceding actions was responsible for getting the reward and to what extent.
    -  explore-exploit dilemma: Once you have figured out a strategy to collect a certain value of rewards, should you stick with it or experiment with something that could result in even bigger rewards?
    - eligibility traces help bridge the gap between events and training information. Like TD methods themselves, eligibility traces are a basic mechanism for temporal credit assignment.
...
An agent at state s, takes action a, and the environment produces a reward r, and an observation that the state of the agent is now s_t+1. RL methods are employed to address two related problems:
...
- The problem is to find a policy function (i.e. density function of joint P(s,a)) that generates a series of actions to get maximum reward in the future.
...
##### Bootstraping in TD learning
Imagine an RNN where each time step prediction is $V_t$ and the value of the target after $m$ time steps is $z$. We'd usually have the supervised learning settings where the $z$ is known and we can calculate the error $z - V_t$ and backprop it throught all time steps. The error used in backprop through time is just the sum of all weight updates through the $m$ time steps $\sum_{t=1}^m \Delta w_t$. Here the gradient of the error w.r.t. parameters of the value function $V$ is just $\Delta w_t = lr * (z - V_t) * \nabla_w V_t,$ that we use in the gradient descent update rule $w \leftarrow w + \sum_{t=1}^m \Delta w_t$. 

However, in on-policy learning settings, we don't know the target $z$ until the end of the episode. The insight of TD learning is that the error at current time step, $z - V_t$, can be represented as the sum of changes in predictions on adjacent time steps, namely $z - V_t = \sum_{k=t}^m (V_{k+1} - V_{k})$. This can be verified by merely expanding the sum and observing that all terms cancel, leaving only $z - V_t$. Replacing this into the update rule, we can estimate the gradient for previous states using the value of the current state. 

$\Delta w_t = \alpha (V_{t+1} - V_t) \sum_{k=1}^t \lambda^{t-k} \Delta_w V_k$

We need at least two successive predictions to form the error term, if we generalize this to $\lambda$ successive predictions for estimating the gradient, we get the $TD(\lambda)$ algorithm, $\lambda$ exponentially discounts past gradinets. In the simplest two successive pridictions case, we must already be in state $s_{t+1}$ to obtain our prediction V_{t+1}, but learning is occurring for prediction $V_t$ (i.e. $w_t$ is being updated). Learning is occuring retroactively for already visited states. This is accomplished by maintaining the gradient $\Delta_w V_k$ as a running sum. When the error appears at time $t + 1$, we obtain the appropriate weight changes for updating all of our predictions for previous input states by multiplying this error by the summed gradient. The update rule is, in effect, updating all preceding predictions to make each closer to prediction $V_{t+1}$ for the current state by using the difference in the successive prediction values, hence the name Temporal-Difference learning. If we continue this until the end of the episode when we see target $z$, we have effectively minimized the actual error but online increamentally instead of waiting till the end of the episode to update. TD(0) produces the same weight changes as would a supervised method. Specifically, $\lambda$ determines the extent to which the prediction values for previous observations are updated by errors occuring on the current step. In other words, it tracks to what extent previous predictions are eligible for updating based on current errors. Therefore, we call the sum $e_t = \sum_{k=1}^t \lambda^{t-k} \Delta_w V_k$ the eligibility trace at time $t$. Eligibility traces are the primary mechanisms of temporal credit assignment in TD learning. That is, credit (or “blame”) for the TD error occurring on a given step is assigned to the previous steps as determined by the eligibility trace. For high values of λ, predictions occurring earlier in a sequence are updated to a greater extent than with lower values of λ for a given error signal on the current step. Eligibility traces in reinforcement learning are used as a bias-variance trade-off and can often speed up training time by propagating knowledge back over time-steps in a single update.

###### an example to Monte Carlo, Dynamic programming and TD in perspective
...
- discounted expected reward: if your environment gives you a local reward for each (state, action) pair,  you need to calculate the returns and maximize that which is the sum of discounted rewards.  starting at the end of episode, discount the sum of rewards of future actions and add to each local reward.
```python
...
# and then normalize discounted expected reward 
...
```python
returns = [R for _ in range(len(policy.saved_log_probs))]
```
...
- Actor and Critic Losses: This loss is just the sum of reinforce loss and the value (critic) loss. The only difference is that instead of using the pure discounted expected reward in reinforce, we reinforce taken actions by the advantage  difference between discounted expected reward and value. That difference is used to adjust the expected value of that action from that state.
```python
for (log_prob, value), r in zip(saved_actions, returns):

    advantage = r - value.item()
    actor_reinforce_loss  = -(log_probs * advantage.detach()).mean()
    critic_loss = advantage.pow(2).mean() # F.smooth_l1_loss(value, torch.tensor([r]))

    policy_losses.append(actor_reinforce_loss)
...
- a NN learns the prediction problem and then a smaller controller model learns to preform a task (focus on credit assignment) using the prediction model. 
...
Behavioral cloning learns a policy as a supervised learning problem over state-action pairs from expert trajectories (e.g. recording human driver data and then mapping the front stereo camera to the steering angle of the car). It requires large amount of data, is prone to compounding error caused by covariant shift, and fails to capture the intuition that the actions made by the expert have a goal. 



## Inverse Reinforcement Learning

The goal is to infer the cost function underlying demonstrated behavior. This cost function explains the expert behaviour but doesn't directly tell the learner how to act.  IRL is expensive to run, as it is learning RL in an inner loop (in each iteration you first learn an estimate for the reward function, and then have to do RL based on that reward to learn policies), and can also diverge for locally optimal RL cost.

### Maximum Entropy IRL

The principle of maximum entropy states that, given prior data, consider the set of all trial probability distributions that would encode the prior data. According to this principle, the distribution with maximal information entropy is the best choice.
...
- It's a model-free immitation learning algo. that obtains significant performance gains over existing model-free methods in imitating complex behaviors in large, high-dimensional environments. Certain instantiations of the framework will draw an analogy between imitation learning and generative adversarial networks.
