...
- I tend to think of a probability distribution as additional dimention that defines a normalized energy surface over the domain of the variables. Consider two 1d random variables $$x$$ and $$y$$. The probablity distribution over both these variables (joint distribution $$p(x,y)$$) is a normalized 3d surface that defines how probable each point in the $$x-y$$ plane is. 
	+ If we cut this surface at a certain point along $$y$$ dimention, we get the conditional probability of the $$x$$ dimention given that value of the $$y$$ dimention i.e. $$p(x|y = value)$$. Usually, what we mean by conditional probability $$p(x|y)$$ is a function that can give the conditional distribution on $$x$$ for any given value of $$y$$. 
	+ If we multiply this function (conditional distribution) by how likely a cut on the domain of $$y$$ is, according to the distribution of probability on $$y$$, expressed by $$p(y)$$, then we get the joint distribution again i.e. $$p(x,y) = p(y) p(x|y)$$. 
	+ On the other hand, if we sum the joint distribution $$p(x,y)$$, at all possible values of $$y$$, we arrive at a probability distribution that is only dependent on the $$x$$ dimension i.e. $$p(x) = \sum_y p(x,y)$$. This is called the marginal distribution over $$x$$ i.e. $$p(x)$$ and what we did was marginalize out the $$y$$ random variable to get the marginal probability $$p(x)$$ out of the joint distribution $$p(x,y)$$.
...
## Expected Value
EV of a distribution is the mean value in the long run (equals to average in infinity). Expectation of a function $$f(x)$$ over a probability distribution on the argument $$p(x)$$ is the mean value that function $$f$$ takes on when its argument $$x$$ is drawn from distribution $$P(x)$$ and is calculated as the weighted sum of the function values $$f(x)$$ weighted by the argument $$x$$! E[.] is a linear operator.
