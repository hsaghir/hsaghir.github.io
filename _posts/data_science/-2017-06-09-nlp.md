...
#### Extractive:
Most extractive methods to date identify sentences based on human-engineered features such as sentence position and length (Radev et al., 2004), the words in the title, the presence of proper nouns, content features such as word frequency (Nenkova et al., 2006), and event features such as action nouns. Then it's treated as a retrieval task using sth like a classifier.
...
- examples: contains examples of how to display data, training and evaluation. 
    +  display data: display data from a particular task provided on the command-line.
...
- To be able to use ParlAI, one needs to understand the following:
    + argument parsing 
    + data flow 
    + and how to process mini-batches. 

ParlAI is conceptually setup like an RL problem setup. Agents interact with each other and pass messages around. An agent is either a student (learn) or teacher(provide data). 

1. First we need to define the world and agents.

```python 
# opt contains parameters and an id
...
2. Then we need to define how we want the agents to interact inside the world. We do that inside the "world.parley()" function which defines how one step of message exchange between the agents looks like (batch or otherwise). Here is an example: 
```python
def parley(self):
...
3. then we need to define a main loop for exchanges to happen inside. Each to call "world.parley()" in the main loop can execute one step of training / evaluation.
```python
...
as an example, training the drqa model on squad dataset with batch size 32 is as easy as running the following command:
```bash
...
There are a couple of pre-defined worlds that let agents interact in a set of common pre-defined ways. 
    ...
    + DialogPartnerWorld(World) provides a two-agent turn-based dialog setting. switches back and forth between two agents, giving each agent one chance to speak per turn and passing that back to the other one.
    + MultiAgentDialogWorld(World) provides a multi-agent setting. each agent gets a turn in a round-robin fashion, receiving as input the actions of all other agents since his last acted.
    ...
    + All worlds are initialized with the following parameters:
        * opt – contains any options needed to set up the agent. usually following command-line arguments:
            -m: Class name of the model. egparlai.parlai_agents.<directory_name>.<file_name>:<class_name>
            -t: Task name of ParlAI. eg babi:Task1k:1orbabi,cbt
            -mf: File name for saving or loading the model.
            -e: Epoch number.
            -rnn: Select GRU or LSTM.
            -bs: Batch size at batch learning.
            -hs: Number of dimensions of hidden layer vector and embedding vector.
            -nl: Number of hidden layers.
            -lr: Learning rate.
            -dr: Drop-out rate.
            -ltim: Display logs every n seconds.
            -vtim: Perform verification every n seconds.
        * agents – the set of agents that should be attached to the world. This is ignored in the case of sharing, and the shared parameter is used instead to initialize agents.
        * shared (optional) – if not None, contains any shared data used to construct this particular instantiation of the world. This data might have been initialized by another world, so that different agents can share the same data.

Defining an Agent commonly involves implementing the following methods:
    + __init__(): Initialize the class. In batch learning, only the learning part is initialized. 
    + observe(): Observe training data, concatenate episodic samples and return them.
    + batchify(): Convert the observed training into a variable that can be processed by the learning model.
    + batch_act(): Behavior in batch learning. Inputs training data for batch size etc as arguments and returns output for batch size.
    + act(): Behavior in online learning.
    + save(): Saves learning models and so on.
    + load(): Load saved learning model etc.

##### Building an agent
An student agent is usually a learn-able model (e.g. a seq2seq model) that receives an input batch through a python dictionary using observe(), passes it through the network, does an optimizer step and produces an output that is sent out through a python dictionary via act(). The network model can be implemented in any DL framework. 

A teacher agent is usually a dataset that samples the data, picks a batch, and sends it to the student agent using the act() function. It also receives the output of the student agent via observe() and uses it for evaluating the student model. 

students learn only agents in the model world, agents in the shared data world do not participate in learning.

In order to initialize an agent we need to do:
    1. first we inherit the parent class (Agent)
    2. Then we check if the parameters are shared from another environment (in which case we only copy values). Otherwise we initialize the model from scratch. 
...
Let's say batch_size=4. ParlAI actually creates four shared versions of the teacher agent, and four shared versions of the student agent. All teacher instances inherit data from their parent but have their own states (e.g. their current example). Shared student agents concatenate previous examples they’ve seen in the same episode.

Observations are generated using "act()" on each teacher, then passing them to shared students using their "observe()". Student agents aggregate observations until an "episode_done" flag is seen when all the minibatch size is observe. Then the minibatch is passed to a "batch_act()" to leverage minibatch learning.
...
Then we simply call the "batch_act" function from the "act" function. "batch_act" receives list of "batch_size" observations and returns a list of the same length with agent’s replies. This is how we need to set it up: 
...
8- greedy scorer
    ...
    - data: 6k convai + 2k inttelab conv
