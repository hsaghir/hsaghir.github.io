...
- [Neural extractive summarization](Cheng and Lapata,2016; Nallapati et al., 2017; Narayan et al., 2017; Yasunaga et al., 2017; Narayan et al., 2018;)
...
Neural-based approaches usually have document length limits.
...
- Attention serves the need for translating long sequences to long sequences since a single last vector of encoder can't remember everything about the sequence. Attention also serves the need for allignment of input and output sequences by learning which part of the input sequence is relevant to which part of the output sequence. 
    ...
    + Conditioned ond the concatenation of the context vector and decoder input, the decoder now can compute its next state. 
...
- Neural approach: a hierarchical encoder to derive a meaning representation and an attention mechanism to directly select sentence/words from input (i.e. pointer networks). 
