...
##### Batch training 
Let's say batch_size=4. ParlAI actually creates four shared versions of the teacher agent, and four shared versions of the student agent. Using the control "if not shared", we initialize the first instance of the agent (e.g. the encoder/decoder), and other instances just share the same parameters and do not participate in learning.  All teacher instances use the same dataset but have their own states (e.g. their current example). Shared student agents concatenate previous examples they’ve seen in the same episode until . Every time we call "parley()" on this BatchWorld (shared data world), this following happens:
    1. each of the batchsize shared "Task10kTeacher" perform "act()" individually.
    2. each of the batchsize shared "Seq2seqAgent" perform "observe()" individually.
    3. The "Seq2seqAgent" executes "batch_act()" on all the batchsize examples. (if agent implements "batch_act()", "act()" will not be called as long as "batchsize > 1".)
    4. each "Task10kTeacher" performs "observe()" individually and stores result.
