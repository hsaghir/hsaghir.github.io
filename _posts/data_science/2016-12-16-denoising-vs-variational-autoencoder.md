...
Now that we have a defined a loss function, we need the gradient of the loss function, $$ \delta E_q[-log q(z|x)+p(x,z)] $$ to be able to use it for optimization with SGD. The gradient isn't easy to derive analytically but we can estimate it by using MCMC to directly sample from $$q(z|x)$$ and estimate the gradient. This approach generally exihibits large variance since MCMC might sample from rare values.
